...
//
{
	...
	landuse_getInitBuildup(sp, Subcatch[j].landFactor,  Subcatch[j].initBuildup,
...
//
{
    ...
    for (p = 0; p < sp->Nobjects[POLLUT]; p++) OutflowLoad[p] = 0.0;
    ...
    if ( Subcatch[j].lidArea > 0.0 )
    {
        lid_addDrainLoads(sp, j, Subcatch[j].newQual, tStep);
    }
}
