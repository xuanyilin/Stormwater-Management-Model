...
static double rect_open_getdSdA(SWMM_Project *sp, TXsect* xsect, double a);
...
static double rect_triang_getdSdA(SWMM_Project *sp, TXsect* xsect, double a);
...
static double rect_round_getSofA(SWMM_Project *sp, TXsect* xsect, double a);
static double rect_round_getdSdA(SWMM_Project *sp, TXsect* xsect, double a);
...
static double mod_basket_getdSdA(SWMM_Project *sp, TXsect* xsect, double a);
...
static double trapez_getdSdA(SWMM_Project *sp, TXsect* xsect, double a);
...
static double triang_getdSdA(SWMM_Project *sp, TXsect* xsect, double a);
...
static double getQcritical(SWMM_Project *sp, double yc, void* p);
static double getYcritEnum(SWMM_Project *sp, TXsect* xsect, double q, double y0);
static double getYcritRidder(SWMM_Project *sp, TXsect* xsect, double q, double y0);
...
//
{
    ...
    case FILLED_CIRCULAR:
        ...
        xsect->sBot  = xsect_getWofY(sp, xsect, xsect->yBot);
    ...
    case MOD_BASKET:
        ...
        xsect->sMax = xsect_getSofA(sp, xsect, Amax[MOD_BASKET]*xsect->aFull);
        break;
    ...
    case PARABOLIC:
        ...
        xsect->rFull = xsect_getRofY(sp, xsect, xsect->yFull);
        xsect->sFull = xsect->aFull * pow(xsect->rFull, 2./3.);
        xsect->sMax  = xsect->sFull;
        break;
...
//
{
    int     index = sp->Curve[xsect->transect].refersTo;
...
double xsect_getSofA(SWMM_Project *sp, TXsect *xsect, double a)
//
//  Input:   xsect = ptr. to a cross section data structure
//           a = area (ft2)
...
//
{
    ...
    switch ( xsect->type )
    {
      ...
      case RECT_CLOSED:
        return rect_closed_getSofA(sp, xsect, a);
      ...
      case RECT_ROUND:
        return rect_round_getSofA(sp, xsect, a);

      default:
        if (a == 0.0) return 0.0;
        r = xsect_getRofA(sp, xsect, a);
...
double xsect_getYofA(SWMM_Project *sp, TXsect *xsect, double a)
//
//  Input:   xsect = ptr. to a cross section data structure
//           a = area (ft2)
...
//
{
    ...
    switch ( xsect->type )
    {
      ...
      default:           return xsect_getRofA(sp, xsect, xsect_getAofY(sp, xsect, y) );
    }
}

//=============================================================================

double xsect_getRofA(SWMM_Project *sp, TXsect *xsect, double a)
//
//  Input:   xsect = ptr. to a cross section data structure
//           a = area (ft2)
//  Output:  returns hydraulic radius (ft)
...
//
{
    ...
    switch ( xsect->type )
    {
      ...
      case FILLED_CIRCULAR:
      case CUSTOM:
        return xsect_getRofY(sp, xsect, xsect_getYofA(sp, xsect, a) );
      ...
      default:
        cathy = xsect_getSofA(sp, xsect, a);
...
double xsect_getdSdA(SWMM_Project *sp, TXsect* xsect, double a)
//
//  Input:   xsect = ptr. to a cross section data structure
//           a = area (ft2)
...
//
{
    switch ( xsect->type )
    {
      ...
      case RECT_CLOSED:
        return rect_closed_getdSdA(sp, xsect, a);

      case RECT_OPEN:
        return rect_open_getdSdA(sp, xsect, a);

      case RECT_TRIANG:
	return rect_triang_getdSdA(sp, xsect, a);

      case RECT_ROUND:
	return rect_round_getdSdA(sp, xsect, a);

      case MOD_BASKET:
	return mod_basket_getdSdA(sp, xsect, a);

      case TRAPEZOIDAL:
	return trapez_getdSdA(sp, xsect, a);

      case TRIANGULAR:
	return triang_getdSdA(sp, xsect, a);

      default: return generic_getdSdA(sp, xsect, a);
    }
}
...
//
{
    ...
    switch ( xsect->type )
    {
      ...
      default:
        ...
        if ( r >= 0.5 && r <= 2.0 )
            y = getYcritEnum(sp, xsect, q, y);

        // --- otherwise use Ridder's root finding method
        else y = getYcritRidder(sp, xsect, q, y);
    }
...
//
{
    ...
    s = xsect_getSofA(sp, xsectStar->xsect, a);
    *f = s - xsectStar->s;
    *df = xsect_getdSdA(sp, xsectStar->xsect, a);
}
...
double generic_getdSdA(SWMM_Project *sp, TXsect* xsect, double a)
//
//  Input:   xsect = ptr. to cross section data structure
//           a = area (ft2)
//  Output:  returns derivative of section factor w.r.t. area (ft^2/3)
...
//
{
    ...
    return (xsect_getSofA(sp, xsect, a2) - xsect_getSofA(sp, xsect, a1)) / (a2 - a1);
}
...
double getQcritical(SWMM_Project *sp, double yc, void* p)
//
...
//
{
    ...
    xsectStar = (TXsectStar *)p;
    a = xsect_getAofY(sp, xsectStar->xsect, yc);
    w = xsect_getWofY(sp, xsectStar->xsect, yc);
    qc = -xsectStar->qc;
...
double getYcritEnum(SWMM_Project *sp, TXsect* xsect, double q, double y0)
//
//  Input:   xsect = ptr. to cross section data structure
...
//
{
    ...
    xsectStar.xsect = xsect;
    xsectStar.qc = 0.0;
    q0 = getQcritical(sp, i1*dy, &xsectStar);
    ...
    if ( q0 < q )
    {
        ...
        for ( i = i1+1; i <= 25; i++)
        {
            ...
            qc = getQcritical(sp, i*dy, &xsectStar);
            if ( qc >= q )
            {
...
double getYcritRidder(SWMM_Project *sp, TXsect* xsect, double q, double y0)
//
//  Input:   xsect = ptr. to cross section data structure
//           q = critical flow rate (cfs)
//           y0 = estimate of critical depth (ft)
...
//
{
    ...
    q2 = getQcritical(sp, y2, &xsectStar);
    if (q2 < q ) return xsect->yFull;

    // --- evaluate critical flow at initial depth guess y0
    //     and at 1/2 of full depth
    q0 = getQcritical(sp, y0, &xsectStar);
    q1 = getQcritical(sp, 0.5*xsect->yFull, &xsectStar);
    ...
    yc = findroot_Ridder(sp, y1, y2, 0.001, getQcritical, &xsectStar);
    return yc;
}
...
double rect_closed_getSofA(SWMM_Project *sp, TXsect* xsect, double a)
{
    ...
    return a * pow(xsect_getRofA(sp, xsect, a), 2./3.);
}

double rect_closed_getdSdA(SWMM_Project *sp, TXsect* xsect, double a)
{
    ...
    if ( alpha <= 1.0e-30 ) return generic_getdSdA(sp, xsect, a);
...
double rect_open_getdSdA(SWMM_Project *sp, TXsect* xsect, double a)
{
    double r, dPdA;

    // --- for small a/aFull use generic central difference formula
    if ( a / xsect->aFull <= 1.0e-30 ) return generic_getdSdA(sp, xsect, a);
...
double rect_triang_getdSdA(SWMM_Project *sp, TXsect* xsect, double a)
{
...
double rect_round_getSofA(SWMM_Project *sp, TXsect* xsect, double a)
{
...
double rect_round_getdSdA(SWMM_Project *sp, TXsect* xsect, double a)
{
    ...
    else return generic_getdSdA(sp, xsect, a);
}
...
double mod_basket_getdSdA(SWMM_Project *sp, TXsect* xsect, double a)
{
    double r, dPdA;
...
double trapez_getdSdA(SWMM_Project *sp, TXsect* xsect, double a)
{
    double r, dPdA;
    // --- use generic central difference method for very small a
    if ( a/xsect->aFull <= 1.0e-30 ) return generic_getdSdA(sp, xsect, a);
...
double triang_getdSdA(SWMM_Project *sp, TXsect* xsect, double a)
{
    double r, dPdA;
