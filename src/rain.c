...
static int  findNWSOnlineFormat(SWMM_Project *sp, FILE *f, char *line);
...
static void saveRainfall(SWMM_Project *sp, DateTime date1, int hour,
        int minute, float x,
            char isMissing);
static void setCondition(SWMM_Project *sp, char flag);
static int  getNWSInterval(char *elemType);
static int  parseStdLine(SWMM_Project *sp, char *line, int *year, int *month,
...
//
{
    ...
    if ( count > 0 ) report_writeRainStats(sp, -1, &rn->RainStats);
    for ( i = 0;  i < count; i++ )
    {
    ...
    for ( i = 0; i < sp->Nobjects[GAGE]; i++ )
    {
        ...
        if ( addGageToRainFile(sp, i) )
        {
            ...
            filePos1 = ftell(sp->Frain.file);
            filePos2 = filePos3;
            report_writeRainStats(sp, i, &rn->RainStats);
        }
    }
...
//
{
    ...
    *hdrLines = 0;
    for (lineCount = 1; lineCount <= maxCount; lineCount++)
    {
        ...
        if ( n == 3 )
        {
            rn->Interval = getNWSInterval(elemType);
            rn->TimeOffset = rn->Interval;
            if ( rn->Interval > 0 )
            {
        ...
        {
            fileFormat = findNWSOnlineFormat(sp, f, line);
            break;
        }
        ...
        if ( parseStdLine(sp, line, &year, &month, &day, &hour, &minute, &x) )
        {
            fileFormat = STD_SPACE_DELIMITED;
            rn->RainType = sp->Gage[i].rainType;
            rn->Interval = sp->Gage[i].rainInterval;
            if ( sp->Gage[i].rainUnits == SI ) rn->UnitsFactor = 1.0/MMperINCH;
            rn->TimeOffset = 0;
            rn->StationID = sp->Gage[i].staID;
            break;
        }
        (*hdrLines)++;

    }
    if ( fileFormat != UNKNOWN_FORMAT ) sp->Gage[i].rainInterval = rn->Interval;
    return fileFormat;
}

//=============================================================================

int findNWSOnlineFormat(SWMM_Project *sp, FILE *f, char *line)
//
...
//
{
    ...
    rewind(f);
    rn->RainStats.startDate  = NO_DATE;
    rn->RainStats.endDate    = NO_DATE;
    rn->RainStats.periodsRain = 0;
    rn->RainStats.periodsMissing = 0;
    rn->RainStats.periodsMalfunc = 0;
    rn->RainAccum = 0.0;
    rn->AccumStartDate = NO_DATE;
    rn->PreviousDate = NO_DATE;
...
//
{
    ...
    switch ( fileFormat )
    {
      ...
      case NWS_SPACE_DELIMITED:
        if ( rn->hasStationName ) nameLength = 31;
      ...
      case NWS_ONLINE_60:
      case NWS_ONLINE_15:
        if ( lineLength <= rn->DataOffset + 23 ) return 0;
        if ( sscanf(&line[rn->DataOffset], "%4d%2d%2d", &y, &m, &d) < 3 ) return 0;
        k = rn->DataOffset + 8;
        break;
    ...
    while ( k < lineLength )
    {
        ...
        switch ( fileFormat )
        {
          ...
          case NWS_ONLINE_15:
			  ...
			  n += readNwsOnlineValue(&line[rn->ValueOffset], &v, &flag1);         //(5.1.011)
        ...
        setCondition(sp, flag1);
        if ( rn->Condition == DELETED_PERIOD ||
             rn->Condition == MISSING_PERIOD ||
        ...
        if ( flag1 == 'a' )
        {
            rn->AccumStartDate = date1 + datetime_encodeTime(hour, minute, 0);
        }
        else if ( flag1 == 'A' )
        {
        ...
        if ( flag1 == 'A' || flag1 == '}' || flag1 == ']') rn->Condition = 0;
    }
    return result;
}
...
void  setCondition(SWMM_Project *sp, char flag)
{
    TRainShared *rn = &sp->RainShared;
    ...
    switch ( flag )
    {
      case 'a':
      case 'A':
        rn->Condition = ACCUMULATED_PERIOD;
        break;
      case '{':
      case '}':
        rn->Condition = DELETED_PERIOD;
        break;
      case '[':
      case ']':
        rn->Condition = MISSING_PERIOD;
...
//
{
    ...
    if ( date2 <= rn->PreviousDate )
    {
        report_writeErrorMsg(sp, ERR_RAIN_FILE_SEQUENCE, sp->Gage[rn->GageIndex].fname);   //(5.1.010)
        report_writeLine(sp, line);
        return -1;
    }
    rn->PreviousDate = date2;

    switch (rn->RainType)
    {
      case RAINFALL_INTENSITY:
        x = x * rn->Interval / 3600.0f;
        break;
...
int parseStdLine(SWMM_Project *sp, char *line, int *year, int *month, int *day,
...
//
{
    ...
    if ( n < 7 ) return 0;
    if ( rn->StationID != NULL && !strcomp(token, rn->StationID) ) return 0;
    return 1;
}
...
//
{
    ...
    if ( rn->AccumStartDate == NO_DATE ) return;

    // --- find number of recording intervals over accumulation period
    date2 = date1 + datetime_encodeTime(hour, minute, 0);
    n = (datetime_timeDiff(date2, rn->AccumStartDate) / rn->Interval) + 1;

    // --- update count of rain or missing periods
    if ( v == 99999 )
    {
        rn->RainStats.periodsMissing += n;
        return;
    }
    rn->RainStats.periodsRain += n;
    ...
    if ( x > 0.0f )
    {
        date2 = datetime_addSeconds(rn->AccumStartDate, -rn->TimeOffset);
        if ( rn->RainStats.startDate == NO_DATE ) rn->RainStats.startDate = date2;
        for (j = 0; j < n; j++)
        {
            fwrite(&date2, sizeof(DateTime), 1, sp->Frain.file);
            fwrite(&x, sizeof(float), 1, sp->Frain.file);
            date2 = datetime_addSeconds(date2, rn->Interval);
            rn->RainStats.endDate = date2;
        }
    }
...
//
{
    ...
    if ( isMissing ) rn->RainStats.periodsMissing++;
    else             rn->RainStats.periodsRain++;

    // --- if rainfall not missing then save it to rainfall interface file
    if ( !isMissing )
    {
        seconds = 3600*hour + 60*minute - rn->TimeOffset;
